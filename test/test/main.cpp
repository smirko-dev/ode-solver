...
int main(int argc, char** argv)
{
    ...
    static constexpr float_t e{0.0001F};
    for (float_t t{0.0F}; t < 1.F; t += dt)
    {
        ...
        {
            errors = true;
            std::cerr << "Mismatch Euler(" << t << ")=" << y1.getParams()[0u] << " != sin(" << t << ")=" << y << std::endl;
        }
        if (!equal(y2.getParams()[0u], y, e))
        {
            errors = true;
            std::cerr << "Mismatch MidPoint(" << t << ")=" << y2.getParams()[0u] << " != " << y << std::endl;
        }
        if (!equal(y3.getParams()[0u], y, e))
        {
            errors = true;
            std::cerr << "Mismatch RungeKutta(" << t << ")=" << y3.getParams()[0u] << " != " << y << std::endl;
        }

        if (!silent)
        {
            std::cout << t << "\t" << y1.getParams()[0u]<< "\t" << y2.getParams()[0u]<< "\t" << y3.getParams()[0u] << "\t" << y << std::endl;
        }
    }
